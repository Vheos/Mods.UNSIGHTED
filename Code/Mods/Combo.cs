...
public class Combo : AMod
{
    ...
    // Settings
    private static ModSetting<float> _duration;
    private static ModSetting<int> _decreaseRate;
    private static ModSetting<bool> _decayRateIsPercent;
    private static ModSetting<int> _gainPerSwordHit;
    private static ModSetting<int> _gainPerAxeHit;
    private static ModSetting<int> _gainPerGunHit;
    private static ModSetting<int> _gainPerShurikenHit;
    private static ModSetting<int> _gainPerParry;
    private static ModSetting<int> _lossPerHitTaken;
    private static ModSetting<bool> _lossPerHitTakenIsPercent;
    private static ModSetting<int> _comboChipBonus;
    private static ModSetting<int> _syringeGainRate;
    private static ModSetting<bool> _syringeGainRateAffectedByCombo;
...
#pragma warning disable IDE0051, IDE0060, IDE1006
    ...
    [HarmonyPatch(typeof(ComboBar), nameof(ComboBar.AddComboValue),
        new[] { typeof(float) }),
        HarmonyPrefix]
    private static bool ComboBar_AddComboValue_Pre(ComboBar __instance, ref float ammountToAddInCombo)
    {
        ...
        else if (Time.time == PlayersManager.instance.players[__instance.playerNum].myCharacter.myCharacterCollider.lastTimePerfectParry)
        ...
        LifeBarsManager.instance.syringeList[__instance.playerNum].AddSyringeValue(syringeGain, false, true);

        ammountToAddInCombo *= 1f + _comboChipBonus / 100f * UnsightedHelpers.instance.NumberOfChipsEquipped("ComboChipA", __instance.playerNum);
    ...
    [HarmonyPatch(typeof(ComboBar), nameof(ComboBar.AddComboValue),
        ...
        HarmonyPrefix]
    private static bool ComboBar_AddComboValue2_Pre(ComboBar __instance, ref int __result, MeleeWeaponClass meleeWeaponClass, bool projectileDamage, bool enemyWasStunned, int damage)
    {
        __instance.CheckMaxComboValue();
        __instance.comboValue.SetClamp(1, __instance.maxComboValue);
        ...
        comboIncrease *= 1f + _comboChipBonus / 100f * UnsightedHelpers.instance.NumberOfChipsEquipped("ComboChipA", __instance.playerNum);
    ...
    private static IEnumerator ComboBar_DecreaseComboBar_Post(IEnumerator original, ComboBar __instance)
    {
    ...
    private static bool ComboBar_OnEnable_Pre(ComboBar __instance)
    => false;

    [HarmonyPatch(typeof(BasicCharacterController), nameof(BasicCharacterController.DeathCoroutine)), HarmonyPostfix]
    private static void BasicCharacterController_DeathCoroutine_Post(BasicCharacterController __instance)
    {
        ComboBar comboBar = LifeBarsManager.instance.comboBarList[__instance.myInfo.playerNum];
