...
public class ParryChallenge : AMod
{
    ...
    // Settings
    private static ModSetting<bool> _allowAttacking;
    private static ModSetting<int> _spawnInterval;
    private static ModSetting<bool> _spawnAnywhere;
    private static ModSetting<int> _spawnDistanceFromPlayers;
    private static ModSetting<bool> _separateFirstTimeRewards;
    private static ModSetting<bool> _resetFirstTimeRewards;
    private static ModSetting<SpawnPreset> _preset;
    private static ModSetting<Vector3> _rewardThresholds;
    private static ModSetting<Vector4>[] _spawnTable;
    ...
    // Privates
    private static readonly Rect SPAWN_RECT = Rect.MinMaxRect(-19, -9, +1, +1);
    private static void ResetFirstTimeRewardsFromConfig()
    {
        if (!_resetFirstTimeRewards)
            return;

        _resetFirstTimeRewards.SetSilently(false);
        UnsightedHelpers.instance.GetPlayerData().dataStrings.RemoveAll(t => t.Contains("EnduranceReward"));
    }
    private static void LoadSpawnPreset()
    {
    ...
    internal static bool IsAnyGymMinigameActive()
    => GymMinigame.instance.TryNonNull(out var gymMinigame)
    && gymMinigame.duringMinigame;
    internal static bool IsParryChallengeActive()
    => IsAnyGymMinigameActive()
    && GymMinigame.instance.currentMinigameType == GymMinigameType.Endurance;
...
#pragma warning disable IDE0051, IDE0060, IDE1006
    ...
    private static IEnumerator GymMinigame_EnduranceMinigame_Post(IEnumerator original, GymMinigame __instance)
    {
        yield return original.MoveNextThenGetCurrent();
        yield return original.MoveNextThenGetCurrent();
        while (PopupManager.instance.currentPopups.Count > 0)
            yield return original.MoveNextThenGetCurrent();

        // Dialogues
        DialogueSystem dialogueSystem = DialogueSystem.instance;
        int dialogueIndex = __instance.numberOfParries.ToFloat().ChooseThresholdValue
        (
            1,
        ...
        // Rewards
        List<string> playerDataStrings = UnsightedHelpers.instance.GetPlayerData().dataStrings;
    ...
    private static IEnumerator GymMinigame_EnemySpawnCycle_Post(IEnumerator original, GymMinigame __instance)
    {
        original.MoveNext();
    ...
    private static bool GymMinigame_EnemyParry_Pre(GymMinigame __instance)
    {
        if (__instance.currentMinigameType != GymMinigameType.Endurance)
            return false;
        ...
        for (int i = _spawnTable.Length - 1; i >= 0; i--)
            ...
            && __instance.numberOfParries >= _spawnTable[i].Value.x)
            {
                FmodMusicController.instance.SetMusicParameter("parrychallenge", i);
                break;
            }
    ...
    private static bool BasicCharacterController_DetectFireInput_Pre(BasicCharacterController __instance)
    ...
    private static bool GymMinigame_GetPositionAwayFromPlayer_Pre(GymMinigame __instance, ref Vector3 __result)
    {
        int i = 0;
        bool isValid = false;
        while (!isValid)
