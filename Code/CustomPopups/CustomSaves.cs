...
{
    // Publics
    internal static void SetSaveSlotsCount(int count)
    {
        // remove old buttons
        ...
        var globalGameData = GlobalGameData.instance;
    ...
    private static SaveSlotPopup _menuPrefab;
    private static int GetSaveSlotID(GameType gameType, int buttonID)
    => (int)gameType * 3
    + buttonID / 3 * 9
    + buttonID % 3;
    private static int GetButtonID(int saveSlotID)
    => saveSlotID / 9 * 3
    + saveSlotID % 9;

    // Settings
    private static readonly Rect SCREEN_RECT = Rect.MinMaxRect(0, 2, 400, 162);
    private static readonly Vector2 BUTTON_SIZE = new(230, 40);
    private static readonly Vector2 BUTTON_SCALE = new(0.75f, 0.75f);
    ...
    private static readonly Dictionary<string, (Vector2 Position, Vector2 Size)> RECT_OVERRIDES_BY_CHILD_NAME = new()
    {
    ...
    private static bool SaveSlotPopup_ChangeGameType_Pre(SaveSlotPopup __instance, int targetTypeId, bool resetPage)
    {
    ...
    private static void SaveSlotPopup_Start_Post(SaveSlotPopup __instance)
    {
        if (__instance.saveSlotButtons.Length <= ORIGINAL_BUTTONS_COUNT)
            return;
    ...
    private static void SaveSlotButton_UpdateInfo_Post(SaveSlotButton __instance)
    {
        ...
        foreach (var childGroup in new[] { __instance.haveSaveStuff, __instance.dontHaveSaveStuff })
            foreach (RectTransform child in childGroup.transform)
            {
                ...
                {
                    if (!overrides.Size.Any(v => v.IsNaN()))
                        child.sizeDelta = overrides.Size;
                    if (!overrides.Position.Any(v => v.IsNaN()))
    ...
    private static bool SaveSlotButton_AdjustSpritePivot_Pre(SaveSlotButton __instance)
